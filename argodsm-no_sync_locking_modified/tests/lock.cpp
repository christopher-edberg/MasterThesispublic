...
/** @brief Checks if no_synchronize locking is working as intended */
TEST_F(LockTest,noSyncCohortTest) {
	std::thread threads[nThreads];
	int* no_sync_counter = argo::conew_<int>(0);
	int* sync_counter = argo::conew_<int>(0);
	argo::globallock::cohort_lock *no_sync_lock = new argo::globallock::cohort_lock(true);
	argo::globallock::cohort_lock *sync_lock = new argo::globallock::cohort_lock(false);
	ASSERT_EQ(0, *counter);
	...
	for (int i = 0; i < nThreads; i++) {
		threads[i] = std::thread(
			increment_counter<argo::globallock::cohort_lock>, sync_lock, sync_counter);
	}
	for (int i = 0; i < nThreads; i++) {
		threads[i].join();
	}
	for (int i = 0; i < nThreads; i++) {
		threads[i] = std::thread(
			increment_counter<argo::globallock::cohort_lock>, no_sync_lock, no_sync_counter);
	}
	for (int i = 0; i < nThreads; i++) {
		threads[i].join();
	}
	argo::barrier();
	ASSERT_EQ(iter * nThreads * argo::number_of_nodes(), *sync_counter);
	ASSERT_GE(iter * nThreads * argo::number_of_nodes(), *no_sync_counter);
	argo::codelete_(sync_counter);
	argo::codelete_(no_sync_counter);
	delete sync_lock;
	delete no_sync_lock;
}
/**
