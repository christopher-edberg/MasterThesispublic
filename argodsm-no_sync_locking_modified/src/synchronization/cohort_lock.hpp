	...
	namespace globallock {
		...
		class cohort_lock {
			private:
				...
				/**
				 * @brief If this is true, no synchronization will be performed on lock or
				 * unlock and is left to the discretion of the user.
				 * @warning ONLY INTENDED FOR SPECIAL USE CASE
				 */
				bool no_synchronize;
			...
			public:
				 ...
				 * @param _no_synchronize If true, this lock does not perform
				 * global synchronization upon handover between ArgoDSM nodes.
				 *
				 ...
				 */
				cohort_lock(bool _no_synchronize = false) :
					has_global_lock(false),
					no_synchronize(_no_synchronize),
					...
					global_lock(new global_lock_type(global_lock_field, no_synchronize)),
				 ...
				 * finally over ArgoDSM nodes.
				 */
				cohort_lock(global_lock_type::internal_field_type* global_l_field,bool _no_synchronize = false) :
					has_global_lock(false),
					no_synchronize(_no_synchronize),
					numanodes(1), // sane default
					numahandover(0),
					nodelockowner(NO_OWNER),
					global_lock_field(global_l_field),
					global_lock(new global_lock_type(global_lock_field, no_synchronize)),
					node_lock(new argo::locallock::ticket_lock())
				{
					int num_cpus = sysconf(_SC_NPROCESSORS_CONF); // sane default
